...
namespace InformationLib.DataAccess
{
	// ---------------------------------------------------------------------------------------------
	/// <!-- EndemeTableFactory -->
	/// <summary>
	...
	public class EndemeTableFactory_old
	{
		...
		{
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn, " DROP TABLE "+EndemeAccess.EnSchema+".Endeme;"
		...
		public static EndemeSet GetEndemeSet_old(string conn, string setName, string tableName, string charColumn, string labelColumn, string descrColumn)
		{
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                ...
                , Throws.Actions, "P");
			return new RichDataTable(cmd, null, tableName, "").ToEndemeSet(setName, charColumn, labelColumn, descrColumn);
		}
		...
		private static RichDataTable GetEndemeSetHeader_old(string conn, string setName, string setVersion)
		{
			...
			//  Get the knowledge table
			// ---------------------------------------------------------------------------
			return new RichDataTable(cmd, null, "EndemeSet", SetIdColumn);
		}
		...
		private static Guid InsertCharacteristic_old(EndemeCharacteristic characteristic, Guid setId, string conn)
		{
			...
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                ...
                , Throws.Actions, "P")
                .AddParameter     ("@CharId", charKey                    )
                .AddParameter     ("@SetId" , setId                      )
                .AddParameter     ("@Letter", characteristic.Letter      )
                .AddParameter_safe("@Code"  , characteristic.Code  ,   16)
                .AddParameter_safe("@Label" , characteristic.Label ,  128)
                .AddParameter_safe("@Descr" , characteristic.Descr , 1024);
            cmd.ExecuteNonQuery();
